...
(function (global, factory) {
  ...
  function map () {
    ...
    var listeners = d3.dispatch('mouseover', 'click', 'mouseout', 'selectionUpdated');
    ...
    map.selectionDispatcher = function (dispatcher) {
      listeners.on('selectionUpdated', dispatcher);
      return map;
    };
    ...
    map.processDispatch = function (dispatchString) {
      if (dispatchString[0] === "filter") {
          ...
          gMap.selectAll('.line').each(function (d) {
            if (lines.includes(d.name)) {
              d3.select(this).classed('filtered', false).attr('stroke', d.color); // Show filtered lines
            } else {
              d3.select(this).classed('filtered', true).attr('stroke', 'grey'); // Hide or de-emphasize unfiltered lines
            }
          });
      }
    };
