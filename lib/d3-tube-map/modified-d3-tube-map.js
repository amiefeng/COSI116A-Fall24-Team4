...
(function (global, factory) {
  ...
  function map () {
    ...
    var listeners = d3.dispatch('mouseover', 'click', 'mouseout', );
    ...
    map.updateSelection = function (dispatchString) {
      if (dispatchString[0] === "filter") {
          const _lines = dispatchString[1]
          gMap.selectAll('.line').each(function (d) {
            const lineSelection = d3.select(this);
            // color all the lines if all lines are selected
            if (_lines[0] === 'All Lines'){
              lineSelection
              .classed("unfiltered", false)
              .attr('stroke', d.color);
            }
            else{
              // Filter based on the selected lines
              if (_lines.includes(d.name)) {
                d3.selectAll('.line').classed("unfiltered", false).attr('stroke', d.color)
              } else{
                lineSelection.classed("unfiltered", true).attr('stroke', "grey")
              }
            }
          });
      }
    ...
    function drawLines() {
      gMap
        ...
        .attr('stroke', 'grey')// defualt
        .on('mouseover', function (d) {
            // Check if the line is green based on color or name
            d3.selectAll(".line")
            .filter(function(line) {
              return line.color === d.color;
            }).attr('stroke', d.color); // Change the stroke color on hover
        })
        .on('mouseout', function () {
          // Reset non-selected lines to gray
          d3.selectAll(".line")
            .filter(function () {
        ...
        .on('click', function(d) {
          // Deselect all lines
          d3.selectAll(".line").classed("selected", false).attr("stroke", "grey");
        
          // Select all lines related to the clicked one
          d3.selectAll(".line")
            .filter(function(line) {
              return line.color === d.color;
          })
          .classed("selected", true) // Mark these lines as selected
          .attr("stroke", d.color); // Set the color
