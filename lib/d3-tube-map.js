// This file is a modfied version of Skamsie's d3-tube-map based of John Walley's d3-tube-map
// https://github.com/johnwalley/d3-tube-map
//https://github.com/skamsie/berlin-ubahn-map/
  ...
  function map() {
    ...
    function extractStations(data) {   
            ...
            data.lines.forEach(function(line) {
              line.nodes.forEach(function(node) {
                if (!stations[node.name]) {
                  stations[node.name] = {
                    label: node.name,
                    x: node.coords[0],
                    y: node.coords[1],
                    lineLabel: false, // Update if needed
                    shiftX: 0,
                    shiftY: 0
                  };
                }
    ...
    function extractLines(data) {
        var lines = data.map(function(line) {
            ...
            name: line.name,
            color: line.color,
            stations: line.nodes.map(node => node.name),
            nodes: line.nodes,
